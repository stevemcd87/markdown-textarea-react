...
function MarkdownTextarea(props) {
  ...
  useEffect(() => {
    ...
    markdownRows = lineBreaks.map(designateElement); // Updates state
  ...
  function designateElement(text) {
    ...
    matchedPattern = searchForMatch(text); // updates htmlTag & elementText if matchedPattern was found

    if (matchedPattern) {
      htmlTag = matchedPattern.htmlTag(text);
      if (htmlTag !== "code" && htmlTag !== "ul" && htmlTag !== "table") elementText = text.replace(text.match(new RegExp(...matchedPattern.regExPattern))[0], "");
    }
